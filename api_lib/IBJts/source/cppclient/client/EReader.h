...
public:
	...
	void start();
    void stop();
    EMutex& getMsgQueueMutex() { return m_csMsgQueue; }
    std::deque<std::shared_ptr<EMessage>>& getMsgQueue() { return m_msgQueue; }
    EDecoder& getProcessMsgsDecoder() { return processMsgsDecoder_; }
};
